...
现在我们知道当n为偶数时全为1（填充满💣），n=1时不变，n%4=3时和3s一样，n%4=1时和5s一样，然后无限重复出现3和5。具体实现的时候，我们可以将该矩阵（M）每次转换的算法描述如下：
1. 计算矩阵M的影响矩阵N（💥波及的范围）
2. 将矩阵N的每行和原矩阵M的相邻行做或操作，然后对结果求反，得3s时的结果
3. 在3s结果上重复以上过程，即得5s结果
...
该题涉及到一些数学概念：
***[排列的奇偶性](https://zh.wikipedia.org/wiki/置换的奇偶性)***
